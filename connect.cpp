...
int init_connect_PSX(const char *hostname, int portno) {
  ...
  if (connect(socketID, (struct sockaddr *)&PSXmainserver,
              sizeof(PSXmainserver)) < 0) {
    return -1;
  } else {
    return socketID;
  }
...
char debugInfo[256];
  ...
  sprintf(debugInfo,"Connecting to PSX main server on: %s:%d\n", PSXMainServer, PSXPort);
  ...
  sprintf(debugInfo,"Connecting to PSX boost server on: %s:%d\n", PSXBoostServer,
         PSXBoostPort);
  printDebug(debugInfo,CONSOLE);
