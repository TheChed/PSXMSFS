...
void H397(char *s, Target *T)
{
  ...
  SimConnect_TransmitClientEvent(hSimConnect, SIMCONNECT_OBJECT_ID_USER, EVENT_PARKING,
				 T->parkbreak, SIMCONNECT_GROUP_PRIORITY_HIGHEST,
...
void H426(char *s, Target *T)
{
  ...
  T->steering = -pos;
  SimConnect_TransmitClientEvent(hSimConnect, SIMCONNECT_OBJECT_ID_USER, EVENT_STEERING,
				 T->steering, SIMCONNECT_GROUP_PRIORITY_HIGHEST,
				 SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);
}
...
void Decodeboost(char *s)
{
  ...
  updatePSXBOOST(flightDeckAlt,heading_true,pitch,bank,latitude,longitude);
}
