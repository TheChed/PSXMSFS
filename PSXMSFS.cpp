...
void CALLBACK SimmConnectProcess(SIMCONNECT_RECV *pData, DWORD cbData, void *pContext)
{
  ...
  case SIMCONNECT_RECV_ID_EXCEPTION: {
    ...
    snprintf(debugInfo, sizeof(debugInfo),
	     "Exception risen: %ld, by sender: %ld at index: %ld, dwLastID: %ld", evt->dwException,
	     evt->dwSendID, evt->dwIndex, dwLastID);
  ...
  case SIMCONNECT_RECV_ID_EVENT: {
    ...
    case EVENT_4_SEC: {
       ...
       */
      if (TCAS_INJECT) {
	SimConnect_RequestDataOnSimObjectType(hSimConnect, DATA_REQUEST_TCAS, TCAS_TRAFFIC_DATA,
...
int init_MS_data(void)
{
  ...
  hr = SimConnect_AddToDataDefinition(hSimConnect, DATA_MSFS, "PLANE HEADING DEGREES TRUE",
				      "radians");
   ...
   *
   */
  hr = SimConnect_AddToDataDefinition(hSimConnect, DATA_MSFS, "GEAR HANDLE POSITION",
				      "percent over 100");
  hr = SimConnect_AddToDataDefinition(hSimConnect, DATA_MSFS, "FLAPS HANDLE INDEX", "number");
  hr = SimConnect_AddToDataDefinition(hSimConnect, DATA_MSFS, "SPOILERS HANDLE POSITION",
				      "position");
...
void SetMSFSPos(void)
{
  ...
  APos.altitude = SetAltitude(PSX_on_ground, PSXBoost.flightDeckAlt, -PSXBoost.pitch,
			      PSXDATA.acftelev, ground_altitude);
...
void usage()
{
  ...
  printf("usage: [-E] [-h] [-v] [-s] [-t][-m IP [-p port]] [-b IP [-c port]]\n");
  printf("\t -h, --help");
