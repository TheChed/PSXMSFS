...
void SetUTCTime(void) {
  ...
  if (UTCupdate && validtime) {
    SimConnect_TransmitClientEvent(hSimConnect, SIMCONNECT_OBJECT_ID_USER,
                                   EVENT_ZULU_HOURS, Tmain.hour,
                                   SIMCONNECT_GROUP_PRIORITY_HIGHEST,
                                   SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);
    SimConnect_TransmitClientEvent(hSimConnect, SIMCONNECT_OBJECT_ID_USER,
                                   EVENT_ZULU_MINUTES, Tmain.minute,
                                   SIMCONNECT_GROUP_PRIORITY_HIGHEST,
                                   SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);
    SimConnect_TransmitClientEvent(hSimConnect, SIMCONNECT_OBJECT_ID_USER,
                                   EVENT_ZULU_DAY, Tmain.day,
                                   SIMCONNECT_GROUP_PRIORITY_HIGHEST,
                                   SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);
    SimConnect_TransmitClientEvent(hSimConnect, SIMCONNECT_OBJECT_ID_USER,
                                   EVENT_ZULU_YEAR, Tmain.year,
                                   SIMCONNECT_GROUP_PRIORITY_HIGHEST,
                                   SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);
...
int init_MS_data(void) {
  ...
  hr = SimConnect_AddToDataDefinition(
      hSimConnect, MSFS_CLIENT_DATA, "PLANE ALT ABOVE GROUND MINUS CG", "feet");
  hr = SimConnect_AddToDataDefinition(hSimConnect, MSFS_CLIENT_DATA,
                                      "PLANE LATITUDE", "radians");
  hr = SimConnect_AddToDataDefinition(hSimConnect, MSFS_CLIENT_DATA,
                                      "PLANE LONGITUDE", "radians");
...
double SetAltitude(int onGround) {
  ...
  if (ground_altitude_avail) {
    ...
    printDebug("Ground elevation from MSFS not available",CONSOLE);
    Qi198SentLand = 0;
    Qi198SentAirborne = 0;
  }
  sprintf(debugInfo,
          ...
          "Set: %.2f",
          MSFS_POS.ground_altitude, MSFSHEIGHT, ctrAltitude,
          ctrAltitude + MSFSHEIGHT);
...
void SetMSFSPos(void) {
  ...
  CalcCoord(Tboost.heading_true + M_PI, 92.5, Tboost.latitude, Tboost.longitude,
            &lat, &longi);
